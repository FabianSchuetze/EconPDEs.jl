...
Denote `F` the finite difference scheme corresponding to a PDE. Solving the PDE corresponds to finding `y` such that `F(y) = 0`, i.e. solving a non linear system. `Ψtc` is a solver especially written for these particular systems.
