...
Denote `F` the finite difference scheme corresponding to a PDE. Solving the PDE corresponds to finding `y` such that `F(y) = 0`, i.e. solving a non linear system. `Ψtc` is a non linear solver especially written for  finite difference schemes corresponding to economics PDE.
